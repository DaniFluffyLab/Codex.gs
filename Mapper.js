...
class Mapper {
  ...
  /**
   * Creates a new Mapper instance to manage a Google Sheets tab as a persistent key-value store.
   ...
   * @param {boolean} [options.enableTypeInference] - Allow Mapper to infer rich types, like Arrays and Objects, from data.
   *
   ...
   * const dbAllColumns = new Mapper("1BxiM...", "Users", "UserID", {
   *     mode: "minimal"
   * });
   * 
   * // mode: "minimal" retrieves line data on demand.
   * const dbOnDemand = new Mapper("1BxiM...", "Users", "UserID", {
   *     mode: "minimal",
   *     columns: ["Price", "Stock"]
   * });
   * 
   * // mode: "full" retrieves all data from the rows at initialization.
   * const dbAllData = new Mapper("1BxiM...", "Users", "UserID", {
   *     mode: "full",
  ...
  constructor(sheetId, tableName, keyColumnName, options = {}) {
     ...
     * @private
     */
    this._logSign = `[ Mapper | SheetID:"${sheetId}" | Table: "${tableName}" ]\n`;
    ...
    this._isCdxProxy = Symbol("isMapperProxy")
   ...
   * * Esta função coordena o acesso simultâneo de múltiplas instâncias do Mapper, garantindo que:
  ...
  _locker(role, action) {
    ...
    const k = {
      age: "Mapper_lockerAge",
      readers: "Mapper_readersCount",
      writing: "Mapper_isWriting"
    }
  ...
  _log(code, info) {
    ...
    switch (code) {
      ...
      // Avisos operacionais
      case 100: console.warn(`${prefix} Another Mapper instance is running a critical task. Awaiting...`); return null;
      case 101: console.warn(`${prefix} Too much data, activating safety mode. Consider requesting fewer columns or using minimal mode with Mapper.search() to increase speed.`); return null;
      ...
      case 200: return Error(`${prefix} ` +
        `The "Google Sheets API" Advanced API is not enabled. To use Mapper library, you need ` +
      ...
      case 240: return Error(`${prefix} Method ${info} is only available on mode = full. Use Mapper.search() instead.`)
      ...
      // Erro de usuário ao interagir com Mapper commitado
      ...
      // Bug no Mapper por sintaxe incorreta
  ...
  */
  ...
  /**
   * Cria um backup preventivo da planilha no Google Drive na pasta Mapper Backups, 
  ...
  _createBackup() {
    try {
      ...
      let bkpFolderIt = thisFolder.getFoldersByName("Mapper Backups")
      let bkpFolder = bkpFolderIt.hasNext() ? bkpFolderIt.next() : thisFolder.createFolder("Mapper Backups")
      let bkpName = `[Mapper Bkp ${now} ${timeZone}] ${thisFile.getName()}`
  ...
  /**
   * Retrieves the configuration values and metadata of the current Mapper instance.
   *
  ...
  /**
   * Removes all elements from the Mapper instance and schedules a full cleanup 
  ...
  /**
   * Removes the specified element from the Mapper instance by key.
   * Schedules the deletion of the corresponding row in the Google Sheets on the next commit.
   * * @param {string} key The key of the element to remove.
   * @returns {boolean} `true` if an element in the Mapper object existed and has been removed, or `false` if the element does not exist.
   */
  delete(key) {
  ...
  *keys() {
   ...
   * @throws {Error} If Mapper is not in mode = full.
   * 
   */
  *values() {
    ...
    if (this._options.mode != "full") throw this._log(240, "Mapper.values()")
  ...
  *entries() {
    ...
    // Rejeita uso do método sem estar no modo full.
    if (this._options.mode != "full") throw this._log(240, "Mapper.entries()")
  ...
  *search(mode, search_for) {
   ...
   * @returns {Mapper} The Mapper instance (for method chaining).
   * @throws {Error} If `key` is not a string or number.
   * @throws {Error} If `value` is not a literal object.
   * @throws {Error} If `value` contains properties not defined in the Mapper schema/columns.
  ...
  commit(enableBackup = false) {
    ...
    /**
     * Converte um valor do Mapper para a estrutura CellData da API avançada do Google Sheets.
     ...
     * * @param {*} value - O valor original proveniente do Mapper (String, Number, Boolean, Date ou JSON).
